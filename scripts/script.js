...
map.on('click', function (e) {
    ...
    if (isInWater) {
        if (!startPoint) {
            ...
            $('#display-dist').html(`Total Distance: ${distanceInMiles} miles`).css('color', 'white');
            drawRout(rout);
            getVessels();
            // Start the boat animation if at least two points are added
            if (rout.length >= 2) {
...
const distanceCalc = (rout) => {
    ...
    // Convert the distance to miles and round to 2 decimal places
     distanceInMiles = parseFloat(totalDistance * 0.000621371).toFixed(2);
...
async function getVessels() {
    try {
        ...
        console.log("Distance before filtering:", distanceInMiles);

        // If distance is undefined or invalid, show all boats
        let filteredBoats;
        if (!distanceInMiles || isNaN(distanceInMiles)) {
            filteredBoats = vessels;
        } else {
            filteredBoats = vessels.filter(vessel => {
                return vessel.max_travel_distance_nautical_miles === 'Unlimited' ||
                       parseFloat(vessel.max_travel_distance_nautical_miles) >= distanceInMiles;
            });
        }
        suitableBoat = filteredBoats;
...
const displayBoats = (boats) => {
    ...
    currentBoats.forEach((vessel, index) => {
        $('.boatCatalog').append(`
                ...
                <img src="${vessel.picture}" class="myVessels card-img-top" alt="...">
                <div class="card-body">
                    ...
                    <button class="addToCart" data-index="${index}" data-bs-toggle="offcanvas" data-bs-target="#offcanvasWithBothOptions">
                        Add to Cart
                    </button>
                </div>
            </div> 
            ...
            <div class="card m-2" style="width: 18rem;" data-boat-index="${index}">
                ...
                <div class="card-body">
                    ...
                    <button class="remove " type="button" data-index="${index}">Remove</button>
                     <a href="#formContainer" type="button" class=" checkoutBtn" data-index="${index}">Checkout</a>
                </div>
            </div> 
        `);
    });
...
<div class="card" style="width: 18rem;">
  <div class="card-body">
    ...
    <form id="bookingForm">
        ...
        <button type="submit" id="submitForm">Book</button>
    </form>
  </div>
</div>
`);
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content" id="myModalCont">
                            ...
                            <div class="modal-footer">
                                <button type="button" class=" btn aboutSectexts closeBtn" data-bs-dismiss="modal" id="close-tab">Close</button>
                                <button type="button" id="finalBook" class="btn checkoutBtn">Confirm Booking</button>
                            </div>
                        </div>
                    </div>
                </div>
...
//a shopping cart icon to reopen the orders
  $(document).ready(function(){
    const myOffCan = $("#offcanvasWithBothOptions");
    const loadOffCan = new bootstrap.Offcanvas(myOffCan)
